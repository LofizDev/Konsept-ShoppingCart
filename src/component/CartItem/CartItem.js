...
import {addToCart, minusFromCart, removeFromCart} from '../../redux/shopping/shopping-action';
  ...
  return (
          <div className="shopping__cart-item">
            ...
            <div class="quantity" style={{display: 'flex'}}>
              <p onClick={() => minusFromCart(item.id)} className="add-btn"
                 style={{fontSize: '24px', color: '#1d1d1d', textAlign: 'center'}}>‒</p>
              <span style={{padding: '0 20px', textAlign: 'center'}}>{item.qty}</span>
              <p onClick={() => addToCart(item.id)} className="minus-btn"
                 style={{fontSize: '24px', color: '#1d1d1d'}}>+</p>
            </div>
