import * as actionTypes from './shopping-types';

const INITIAL_STATE = {
  products: [
    {
      id: 1,
      select: 'newproduct',
      name: 'comfy chair',
      price: '40.00',
      sub: 'Handmade',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist7.jpg'
    },
    {
      id: 2,
      select: 'newproduct',
      name: 'desk decor',
      price: '45.00',
      sub: 'Home',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist5.jpg'
    },
    {
      id: 3,
      select: 'newproduct',
      name: 'tea kettle',
      price: '26.00',
      sub: 'Home',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/03/shoplist02.jpg'
    },
    {
      id: 4,
      select: 'newproduct',
      name: 'wall art',
      price: '49.00',
      sub: 'Art - Style',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/03/shoplist1.jpg'
    },
    {
      id: 5,
      select: 'newproduct',
      name: 'pottery',
      price: '20.00',
      sub: 'Handmade',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist4_home1.jpg'
    },
    {
      id: 6,
      select: 'newproduct',
      name: 'wall paint',
      price: '40.00',
      sub: 'Style',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/s_list_2.jpg'
    },
    {
      id: 7,
      select: 'newproduct',
      name: 'rubber plant',
      price: '14.00',
      sub: 'Seed Plaint',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist3_home1.jpg'
    },
    {
      id: 8,
      select: 'newproduct',
      name: 'soft glow',
      price: '62.00',
      sub: 'Style',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/s_list_1.jpg'
    },
    {
      id: 9,
      select: 'newproduct',
      name: 'bed table',
      price: '36.00',
      sub: 'Style',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist3.jpg'
    },
    {
      id: 10,
      select: 'newproduct',
      name: 'basket',
      price: '25.00',
      sub: 'Home',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist1_home11.jpg'
    },
    {
      id: 11,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/home1_1-1-600x600.jpg',
      name: 'FOOD BOARD',
      sub: 'Kitchen',
      price: '15.00'
    },
    {
      id: 12,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shopslider_40-600x600.jpg',
      name: 'BLUE GLASS',
      sub: 'Kitchen',
      price: '45.00'
    },
    {
      id: 13,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/05/slider_featured_4-600x600.jpg',
      name: 'BOWL',
      sub: 'Decorative',
      price: '39.00'
    },
    {
      id: 14,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/06/slider_featured_60-2-600x600.jpg',
      name: 'PILEA',
      sub: 'Decorative',
      price: '32.00'
    },
    {
      id: 15,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/07/slider_featured_70-600x600.jpg',
      name: 'BLUE LAMP',
      sub: 'Design',
      price: '57.00'
    },
    {
      id: 16,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/05/slider_featured_80-600x600.jpg',
      name: 'BOOK SLING',
      sub: 'Decorative',
      price: '57.00'
    },
    {
      id: 17,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shopslider_40-600x600.jpg',
      name: 'BLUE VASE',
      sub: 'Kitchen',
      price: '87.00'
    },
    {
      id: 18,
      select: 'onlinestore',
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/slider_featured_90-600x600.jpg',
      name: 'PLATE',
      sub: 'Kitchen',
      price: '17.00'
    },
    {
      id: 19,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist10.jpg',
      name: 'SOFT GLOW',
      sub: 'Art - Style',
      price: '20.00'
    },
    {
      id: 20,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist4_home2-1-757x1024.jpg',
      name: 'WOOD CHAIR',
      sub: 'Chair',
      price: '55.00'
    },
    {
      id: 21,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist15_.jpg',
      name: 'ORANGE SOFA',
      sub: 'Handmade',
      price: '85.00'
    },
    {
      id: 22,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/06/slider_featured_30-4-757x1024.jpg',
      name: 'ALOE',
      sub: 'Decorative',
      price: '15.00'
    },
    {
      id: 23,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/home3_1.jpg',
      name: 'WALL CLOCK',
      sub: 'Interior',
      price: '15.00'
    },
    {
      id: 24,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/home3_20.jpg',
      name: 'ORANGE LIGHT',
      sub: 'Decorative',
      price: '35.00'
    },
    {
      id: 25,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/home4_16.jpg',
      name: 'ORANGE VASSE',
      sub: 'Decorative',
      price: '16.00'
    },
    {
      id: 26,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/grouped_01-757x1024.jpg',
      name: 'SAGO',
      sub: 'Office Plants',
      price: '28.00'
    },
    {
      id: 27,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/virtual_01-757x1024.jpg',
      name: 'NATUR',
      sub: 'Plants',
      price: '16.00'
    },
    {
      id: 28,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/big_img01-1.jpg',
      name: 'CORN TREE',
      sub: 'Herbs',
      price: '60.00'
    },
    {
      id: 29,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/04/shoplist6.jpg',
      name: 'ARECA',
      sub: 'Office Plants',
      price: '20.00'
    },
    {
      id: 30,
      img: 'https://konsept.qodeinteractive.com/wp-content/uploads/2020/05/grouped_1.jpg',
      name: 'PALM TREE',
      sub: 'Seed Plaint',
      price: '50.00'
    },
  ],
  cart: [],
  currentItem: null
};
const shopReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    case actionTypes.ADD_TO_CART:
      // Get Item data from producs array
      const item = state.products.find(
              (product) => product.id === action.payload.id
      );
      // Check if Item is in cart already
      const inCart = state.cart.find((item) =>
              item.id === action.payload.id ? true : false
      );
      return {
        ...state,
        cart: inCart
                ? state.cart.map((item) =>
                        item.id === action.payload.id
                                ? {...item, qty: item.qty + 1}
                                : item
                )
                : [...state.cart, {...item, qty: 1}]
      };
    case actionTypes.MINUS_FROM_CART:
      const itemm = state.products.find(
              (product) => product.id === action.payload.id
      );
      const inCartt = state.cart.find((itemm) =>
              itemm.id === action.payload.id ? true : false
      );
      return {
        ...state,
        cart: inCartt
                ? state.cart.map((item) =>
                        itemm.id === action.payload.id && item.qty === 1 ? item : item
                                ? {...itemm, qty: item.qty - 1}
                                : item
                )
                : [...state.cart, {...itemm, qty: item.qty}]
      };
    case actionTypes.REMOVE_FROM_CART:
      return {
        ...state,
        cart: state.cart.filter((item) => item.id !== action.payload.id)
      };
    case actionTypes.LOAD_CURRENT_ITEM:
      return {
        ...state,
        currentItem: action.payload
      };
    default:
      return state;
  }
};
export default shopReducer;
